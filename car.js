function initCar() {
  ...
  document.getElementById('car-add-save').addEventListener('click', async () => {
    ...
    await addCar({ brand, mileage, number, oilHistory: [] });
    carAddModal.classList.remove('active');
    loadCars();
  });
  ...
  document.getElementById('oil-save').addEventListener('click', async () => {
    ...
    if (!oilMileage) { alert('Введите пробег'); return; }

    const car = cars.find(c => c.id === currentCarId);
    if (car) {
      const oilHistory = car.oilHistory || [];
      oilHistory.push({ mileage: oilMileage, date: oilDate, type: oilType, note: oilNote });
      oilHistory.sort((a, b) => new Date(b.date) - new Date(a.date));
      await updateCar(currentCarId, { oilHistory, mileage: Math.max(car.mileage, oilMileage) });
    }
  ...
  async function loadCars() {
    ...
    if (cars.length === 0) {
      carContent.innerHTML = `<div class="empty-state"><div class="empty-icon">🚗</div><p>Нет автомобилей</p><p class="empty-hint">Нажми + чтобы добавить</p></div>`;
      return;
    }
      ...
      return `
        <div class="car-card">
          <div class="car-header">
            <div><h3>${car.brand}</h3>${car.number ? `<p class="car-subtitle">${car.number}</p>` : ''}</div>
            <button class="btn-icon delete-car" data-id="${car.id}">🗑️</button>
          </div>
          <div class="car-mileage-row">
            <span class="car-mileage-label">Пробег</span>
            <span class="car-mileage-value">${formatMoney(car.mileage)} км</span>
          </div>
          ${lastOil ? `
            <div class="oil-info-block">
              <p class="oil-info-title">🛢️ Последняя замена</p>
              <div class="oil-info-row"><span>📅 ${formatDate(lastOil.date)}</span><span>🔧 ${formatMoney(lastOil.mileage)} км</span></div>
              <p class="oil-info-type">🛢️ ${lastOil.type}</p>
              ${lastOil.note ? `<p class="oil-info-note">📝 ${lastOil.note}</p>` : ''}
              <p class="oil-next ${kmLeft > 0 ? 'ok' : 'warn'}">${kmLeft > 0 ? `✅ Следующая замена через ~${formatMoney(kmLeft)} км` : `⚠️ Замена просрочена на ${formatMoney(Math.abs(kmLeft))} км!`}</p>
            </div>
          ` : `<div class="oil-info-block"><p style="color:var(--danger);">Нет записей о замене масла</p></div>`}
          <button class="btn-add-money add-oil-btn" data-id="${car.id}" style="margin-top:14px;">🛢️ Замена масла</button>
          ${oilHistory.length > 1 ? `
            <details class="oil-history"><summary>История замен (${oilHistory.length})</summary><ul>
              ${oilHistory.map(o => `<li><span>📅 ${formatDate(o.date)}</span><span>🔧 ${formatMoney(o.mileage)} км</span><span>🛢️ ${o.type}</span>${o.note ? `<span class="oil-note-full">📝 ${o.note}</span>` : ''}</li>`).join('')}
            </ul></details>
          ` : ''}
        </div>`;
    }).join('');
    ...
    document.querySelectorAll('.delete-car').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        if (confirm('Удалить автомобиль?')) { await deleteCar(btn.dataset.id); loadCars(); }
      });
    });
